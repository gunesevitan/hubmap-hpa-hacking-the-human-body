...
if __name__ == '__main__':
    ...
    dataset_path = settings.DATA / 'external_data' / 'HuBMAP_Kidney_Segmentation'

    for size in [256, 512, 1024]:

        image_filenames = sorted(glob(str(dataset_path / f'tiled_images_{size}' / '*.png')))
        ...
        for image_filename in tqdm(image_filenames):
            ...
            mask_filename = str(dataset_path / f'tiled_masks_{size}' / f'{image_id}.png')
            ...
            metadata.append({
                'id': image_id,
                'organ': 'kidney',
                'data_source': 'HuBMAP_Kidney_Segmentation',
                'stain': 'PAS',
        ...
        df_metadata.to_csv(dataset_path / f'metadata_{size}.csv', index=False)
        logging.info(f'Saved metadata_{size}.csv to {dataset_path}')
